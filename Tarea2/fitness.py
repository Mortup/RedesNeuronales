...
iterations = 0
fitnesses = []
fitnesses.append(fitness_func(top_percent[0]))
...
while fitness_func(top_percent[0]) < 0:
    ...
    fitnesses.append(fitness_func(top_percent[0]))
    if iterations % 100 == 0:
        print fitness_func(top_percent[0])

print "Se ha encontrado la combinacion luego de", iterations, "generaciones."
print top_percent[0]

f, ax = plt.subplots()
ax.plot(range(iterations+1), fitnesses)
ax.set_xlabel('Generaciones')
ax.set_ylabel('Fitness')
ax.set_title('Aprendisaje de nQueen 50x50')
plt.show()
